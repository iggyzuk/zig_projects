...
pub fn build(b: *std.Build) !void {
    ...
    //     const exe_lib = emcc.compileForEmscripten(b, "balls", "src/main.zig", target, optimize);
    ...
    //     const run_option = b.step("run", "Run balls");
    //     run_option.dependOn(&run_step.step);
    //     return;
    // }

    const exe = b.addExecutable(.{ .name = "balls", .root_source_file = .{ .path = "src/main.zig" }, .optimize = optimize, .target = target });
